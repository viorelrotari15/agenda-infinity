...
import { parseIsoOrThrow } from '../../shared/dates/parse-iso';
...
export class AppController {
  ...
  async me(@Headers('authorization') authorization?: string) {
    ...
    if (user.role === 'CLIENT') {
      ...
      return { ...base, interestCategoryIds: interests.map((i) => i.categoryId) };
    }
      ...
      select: {
        ...
        categories: {
          ...
          select: {
            isPrimary: true,
            category: {
              select: { slug: true, nameEn: true, nameRo: true, nameRu: true },
            },
          },
        },
      },
    });
    if (!specialist) return { ...base, specialistProfile: null };
    const { categories, ...rest } = specialist;
    return {
      ...base,
      specialistProfile: {
        ...rest,
        categories: categories.map((row) => ({
          ...
          name: categoryLabel(row.category, locale),
  ...
  @Get('specialists/:id/directory-categories')
  specialistDirectoryCategories(@Param('id') id: string) {
    const locale = getLocale();
    return this.prisma.specialistCategory
      .findMany({
        where: { specialistId: id, category: { active: true } },
        select: {
          isPrimary: true,
          category: {
            select: { slug: true, nameEn: true, nameRo: true, nameRu: true },
      ...
      .then((rows) =>
        rows.map((r) => ({
          ...
          name: categoryLabel(r.category, locale),
  ...
  async availability(
    ...
    const fromDate = parseIsoOrThrow(from);
    const toDate = parseIsoOrThrow(to);
  ...
  async createBooking(
    ...
    const firstStart = parseIsoOrThrow(body.startUtc, 'missing_date');
    ...
    const until = body.recurrence.untilUtc
      ? parseIsoOrThrow(body.recurrence.untilUtc, 'missing_date')
  ...
  async specialistBookings(
    ...
    const fromDate = from ? parseIsoOrThrow(from) : new Date(0);
    const toDate = to ? parseIsoOrThrow(to) : new Date('2999-12-31T00:00:00.000Z');
  ...
  async clientBookingById(
    ...
    return {
      id: booking.id,
      title: `${booking.service.name} · ${booking.specialist.displayName}`,
      start: booking.startUtc.toISOString(),
      end: booking.endUtc.toISOString(),
      status: booking.status,
      serviceName: booking.service.name,
      specialistName: booking.specialist.displayName,
      specialistSlug: booking.specialist.slug,
      durationMinutes: booking.service.durationMinutes,
      recurrenceSeriesId: booking.recurrenceSeriesId,
    };
  }
